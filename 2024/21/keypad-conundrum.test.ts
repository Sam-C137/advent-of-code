...
async function main() {
    ...
    const cache: Record<string, any> = { 1: {}, 2: {} };
    ...
    for (const _code of passwords) {
        ...
        for (let i = 0; i < code.length - 1; i++) {
            ...
            total += Math.min(...paths.map((p) => find_shortest(p, 2)));
        }
...
describe("keypad conundrum", () => {
    test("It should have a total complexity of 203734 at depth 2", async () => {
        const result = await main();
        expect(result).toEqual(203734);
    });
});
